...
const GameResultsTable = () => {
  ...
  // 👇 MODIFIED FUNCTION: Adds a specific 'dayOnly' field to every row
  const generateFullYearData = (dataMap, year) => {
    ...
    for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        const dayStr = String(day).padStart(2, '0'); // This is "01", "02", etc.
        const dateStr = `${dayStr}-${monthStr}-${year}`;
        const foundData = dataMap.get(dateStr);
        
        // Merge found data with our calculated date strings
        // This ensures 'dayOnly' exists regardless of what the API sends
        const rowData = foundData ? { ...foundData } : { games: {} };
        
        rowData.date = dateStr;       // Force consistent DD-MM-YYYY format for keys
        rowData.dayOnly = dayStr;     // Force consistent "01", "02" for display
        ...
        <div className="overflow-auto flex-1 relative scroll-smooth">
          <table className="min-w-full text-center border-collapse">
            <thead className="bg-slate-800 text-white font-bold uppercase text-xs md:text-sm sticky top-0 z-30 shadow-md">
              <tr>
                <th className="p-3 border-r border-slate-600 sticky left-0 z-40 bg-slate-800 min-w-25">Day</th>
              ...
              {currentData.rows.map((row, index) => {
                ...
                return (
                  <tr key={row.date} className={`${rowBg} hover:bg-blue-50`}>
                    {/* 👇 MODIFIED: Uses row.dayOnly directly. No splitting needed. */}
                    <td className={`p-3 font-bold text-gray-700 border-r border-gray-200 sticky left-0 z-20 ${rowBg}`}>
                        {row.dayOnly}
                    </td>
                    {COLUMNS.map((col) => (
